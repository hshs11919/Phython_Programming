...
print({x:words.count(x) for x in words})                                    # ✅ {'apple': 3, 'banana': 2, 'cherry': 1}
count = {}
for word in words :
...
result = {k:"합격" for k,v in scores.items() if v >= 60}                                    # ✅ {'국어': '합격', '수학': '합격', '사회': '합격'}
print(result)
...
print({k: (stock[k]+incoming[k] if k in stock and k in incoming else v) for k,v in {**stock, **incoming}.items()})                                    # ✅ {'연필': 10, '지우개': 9, '노트': 10, '볼펜': 12}
