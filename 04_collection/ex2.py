...
res = ["짝수"if x% 2 == 0 else "홀수"for x in range(1,11)]
print(res)
...
result = [s for s in words if s.find('n') >= 0]
result = [s for s in words if s.count('n')]
...
result = [(scores[0][x]+scores[1][x]+scores[2][x])/3 for x in range(len(scores))]
