...
def swap(a,b):      # 매개변수는 객체의 참조값을 전달(같은 객체를 가리킴)
    a,b=b,a
    print(a,b)      # 2,1 -> 함수 종료 시 지역변수 a,b는 유효하지 않게 됨
...
print(sorted(students, key = lambda s:["name"]))
print(sorted(students, key = lambda s:["score"]))
...
print("/".join(["apple", "banana", "kiwi"])) # apple / banana / kiwi
